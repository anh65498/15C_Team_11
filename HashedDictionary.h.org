...
public:
    ...
    vector<ItemType> getRejectedList() const {return rejectedList;}
    void setRejectedList(const vector<ItemType> rlist );
...
bool HashedDictionary<KeyType,ItemType>::add(const KeyType& searchKey, const ItemType& newItem)
{
    ...
    else
    {
        if(hashTable[hashIndex]->getCount() == bucketSize)
        {
            ...
            rejectedList.push_back(newItem);
            overflow++;
            return false;
        }
        Entry<KeyType, ItemType> * entryToAddPtr = new Entry<KeyType, ItemType>(searchKey,newItem);
...
HashedDictionary<KeyType,ItemType>* HashedDictionary<KeyType,ItemType>::reHash(const HashedDictionary<KeyType,ItemType>* oldHashTable,int newSize,int newBucketSize)
{
    ...
    newTable->setRejectedList(oldHashTable->getRejectedList());
...
template <class KeyType,class ItemType>
void HashedDictionary<KeyType,ItemType>::setRejectedList(const vector<ItemType> rlist)
{
    for(int i=0;i<rlist.size();i++)
    {
        rejectedList.push_back(rlist.at(i));
    }
}
...
int HashedDictionary<KeyType,ItemType>::getLongestBucket()
{
    int max=0;
    
    for (int i = 0; i<tableSize; i++)
    {
        LinkedList<Entry<KeyType,ItemType>*> *ptr = hashTable[i];
        if(ptr != 0)
